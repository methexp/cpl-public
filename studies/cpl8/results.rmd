...
summary(model)
ggpredict(model, terms = ~ tid + stimulus_location_regularity + error) |> plot(show_data = TRUE)
```

Before applying the drift-diffusion model,
we first analyzed response times and error rates from the SRTT
and the responses from the post-experimental interview
with linear models.
...
rt_data <- subset(error_data, !error) |>
  aggregate(
    x = response_time ~ sid + block_pair + term + instructions + stimulus_location_regularity
    , FUN = mean
  ) |>
  label_variables(response_time = variable_label(data$response_time))
...
To summarize, for participants who worked on probabilistic materials, we found robust sequence learning in both RTs and error rates.
...
Although, descriptively, the RT difference between sequence concealed compared with sequence revealed conditions decreased,
...
anova_rt_random <- aov_4(
  ...
  , data = subset(rt_data, block_pair > 0)
  , fun_aggregate = mean
) |>
  apa_print()


# ggpredict(anova_rt_random, terms = ~ instructions + stimulus_location_regularity) |>
#   plot()
...
The main effect of *instructions* was not significant,
`r anova_rt_random$full$instructions`.
*Instructions* entered into an interaction with *stimulus-location regularity*,
`r anova_rt_random$full$instructions_stimulus_location_regularity`:
The RT advantage for regular stimulus locations was bigger if the sequence had been revealed.
All other $p\mathrm{s} \geq .420$.
...
#   subset(rt_data, block_pair > 0)
#   , id = "sid"
#   , factors = c("block_pair", "context", "instructions")
#   , dv = "response_time"
#   # , jit = .3
#   , args_points = list(bg = 1:4, pch = rep(22:23, times = 2))
#   , args_error_bars = list(length = .02)
...
anova_rt_mixed_deterministic <- aov_4(
  formula = response_time ~ instructions + (block_pair * context | sid)
  , data = subset(rt_data, block_pair > 1)
  , fun_aggregate = mean
) |>
  apa_print()

anova_errors_mixed_deterministic <- aov_4(
  formula = response_time ~ instructions + (block_pair * context | sid)
  , data = subset(error_data, block_pair > 1)
  , fun_aggregate = mean
) |>
  apa_print()
```
...
```{r rt-errors, fig.cap = "Response times for error responses. Error bars represent 95% within-subjects confidence intervals."}
...
We also analyzed response times (see Figure\ \@ref(fig:rt-errors)) for error responses (again excluding post-error trials) and
...
anova_free_recall_model <- aov_4(free_recall_correct ~ material * instructions + (1 | sid), data = free_recall)
decriptives_free_recall <- emmeans(anova_free_recall_model, specs = ~ material + instructions) |>
  apa_print()

anova_free_recall <- anova_free_recall_model |>
  apa_print()
...
Participants were asked whether they believed they had been working on materials containing a sequence (and to name that sequence); and then were presented with each stimulus and asked to select the next one in the sequence (i.e., forced choice). 
...
Regarding free-recall performance, we analyzed the number of correctly generated transitions using a
...
`r anova_free_recall$full$material`,
a main effect of *instructions*,
`r anova_free_recall$full$instructions`,
and no interaction,
`r anova_free_recall$full$material_instructions`.
Participants who worked on mixed-deterministic materials recalled more transitions than those who worked on probabilistic materials.
Also, revealing the sequence to participants resulted in more correctly-recalled transitions.
...
```{r results = 'hide'}
models <- qs::qread("studies/cpl8/model_objects/phdm_200.qs")[c(2, 4, 1, 3)]
...
(ref:starting-point-cap) Starting point of the diffusion process. Points represent posterior means of the group-level parameters $\mu^{(\beta)}_{kb}$.
...
off <- Map(
  f = function(x, main, plot_legend) {
    plot(
      x
      , pars = "beta"
      , main = main
      , xlab = "Block pair"
      , ylab = expression("Relative starting point"~beta)
      , ylim = c(0, 1)
      , args_points = list(bg = 1:3)
      , args_legend = list(x = "topleft", title = "", plot = plot_legend)
      , args_violins = list(border = 1:3)
    )
  }
  , x = models
  , main = main
  , plot_legend = c(TRUE, FALSE, TRUE, FALSE)
)
```
...
(ref:drift-rate-cap) Average evidence accumulation (drift rate).  Points represent posterior means of the group-level parameters $\mu^{(\nu)}_{kb}$.
...
off <- Map(
  f = function(x, main, plot_legend) {
    plot(
      ...
      , main = main
      , xlab = "Block pair"
      , ylab = expression("Drift rate"~delta)
      , ylim = c(2, 6)
      , args_points = list(bg = 1:3)
      , args_legend = list(x = "topleft", title = "", plot = plot_legend)
      , args_violins = list(border = 1:3)
...
(ref:boundary-separation-cap) Boundary separation (response caution). Points represent posterior means of the group-level parameters $\mu^{(\beta)}_{kb}$.
...
off <- Map(
  f = function(x, main, plot_legend, cols) {
    cols <- if(any(x$factors$alpha == "context")) c(5, 2) else 2
    plot(
      x
      , pars = "alpha"
      , main = main
      ...
      , ylim = c(.5, 1.1)
      , args_points = list(bg = cols)
      , args_violins = list(border = cols)
      , args_legend = list(x = "topleft", title = "", plot = plot_legend)
    )
  }
  , x = models
...
With probabilistic materials, we found evidence for the absence of sequence-specific effects,
...
We therefore conclude that, if participants performed the SRTT in a stimulus-based fashion,
it was neither stimulus detection nor encoding that mediated the expression of sequence learning.

A similar pattern could be observed in participants who worked on mixed-deterministic materials:
When considering random blocks, we found evidence for an absence of a difference in nondecision time attributable to sequence learning,
...
By contrast, with mixed-deterministic materials, we found strong evidence for a rapid decrease
...
We interpret this finding as indicative that a switch to plan-based action control allowed participants
to largely skip the process of stimulus detection and encoding;
instead, during the RSI, they already prepared a response that was executed as soon as something appeared on the screen.

(ref:nondecision-time-cap) Nondecision times, separated by stimulus-location regularity. Points represent posterior means of the group-level parameters $\mu^{(\theta)}_{kb}$.
...
off <- Map(
  f = function(x, main, plot_legend) {
    plot(
      ...
      , main = main
      , xlab = "Block pair"
      , ylab = expression("Nondecision time"~theta)
      , ylim = c(0, .5)
      , args_points = list(bg = 1:3)
      , args_legend = list(x = "topleft", title = "", plot = plot_legend)
      , args_violins = list(border = 1:3)
...
(ref:response-competition-cap) Response-competition parameter $\xi$, capturing differences in nondecision time between motor-regular and motor-nonregular responses. Positive values imply faster responses for motor-regular responses.  Points represent posterior means of the group-level parameters $\mu^{(\xi)}_{kb}$.
...
To characterize the expression of implicit sequence knowledge, we used the parameter estimates from the group that worked on probabilistic materials in the sequence concealed condition and did not show any indication of explicit knowledge (i.e., the *implicit* subgroup). 
...
if the SRTT was performed in a stimulus-based fashion and participants remained implicit about the sequence,
such implicit learning affected the response-selection process, with a tendency to select the rule-adhering response (starting point $\beta$), and faster evidence accumulation for rule-adhering stimuli (drift rate $\delta$). 
...
indicating that in such a processing mode, the little information that was necessary to select the response (i.e., the appearance of a stimulus on the screen) was processed as efficiently as in random blocks.
Nondecision time\ $\theta$ was dramatically reduced in deterministic blocks (compared with random blocks),
indicating that stimulus detection and/or encoding could be bypassed.
...
(ref:ddm-parameters-subset) DDM parameters for stimulus-based (on the left) versus plan-based SRTT performance (on the right).
...
main <- c("Stimulus-based, implicit", "Plan-based, explicit")
...
par(mfrow = c(5, 2), font.main = 1, las = 1, xpd = TRUE)
i <- "beta"
j <- 1
...
for (i in c("beta", "nu", "alpha", "theta", "xi")) {
  for (j in seq_along(important_models)) {
    ...
    if(is_deterministic) colors$alpha <- c(5, 2) else colors$alpha <- 2
    
    # plot_data <- get_plot_data(important_models[[j]], pars = i)
    colors <- c(1, 3, 5)
    if(i == "alpha")                    colors <- 2
    if(i == "alpha" & is_deterministic) colors <- c(5, 2)
    if(i == "xi")                       colors <- 2
    ...
    #   , args_violins = list(border = colors)
    #   , args_error_bars = list(length = .02)
    ...
    plot(
      important_models[[j]]
      , pars = i
      , main = if(i == "beta") { if(is_deterministic) "Plan-based, explicit" else "Stimulus-based, implicit" }
      , xlab = "Block pair"
      , ylab = parameter_names[[i]]
      , ylim = ylim[[i]]
      , args_points = list(bg = colors)
      , args_legend = list(x = "topleft", title = "")
      , args_violins = list(border = colors)
      , args_error_bars = list(length = .02)
    )
