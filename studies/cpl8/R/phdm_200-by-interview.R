...
data <- readRDS("studies/cpl8/data/data-excluded-with-interview.rds")
data <- subset(
  data
...
ftable(criterion ~  material + instructions, data)

models <- split(data, f = data[, c("material", "instructions", "criterion")]) |>
  lapply(FUN = function(x){

    if(length(unique(x$sid)) < 4) return(NULL)
    ...
    phdm_200(
      ...
      , rt = "response_time"
      , error = "error"
      , adapt  = n_samples(4e3L, debug = 1e2L)
      , burnin = n_samples(1e4L, debug = 1e2L)
      , sample = n_samples(3e3L, debug = 1e2L)
      , thin   = n_samples(1e1L, debug = 1e0L)
      , factors = list(
...
qsave(models, file = "studies/cpl8/model_objects/phdm_200-by-interview.qs")
